...
@editora_route.route('/cadastroAjax', methods=['POST'])
def cadastro_ajax():
    try:
        Editora.salvar(int(request.form.get('id', 0)), request.form.get('nome'))
        lista_editoras = Editora.listar()
        editoras_json = [editora.to_dict() for editora in lista_editoras]
        
        return jsonify({
            'status': 'success',
            'message': 'Editora salva com sucesso!',
            'editoras': editoras_json
        })
